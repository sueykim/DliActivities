...
function keyboardLayout() {
	...
	$(xml).find("lang_tl").each(function(i,val){
	    output = $(this).text(); 
		outputArray.push(output);			    
	});
	var opAryLen = outputArray.length;

	for (k=0; k<opAryLen; k++){
		var outputText = outputArray[k];
		var optLen = outputText.length;

		//alert (k);
		/* */
		for (j=0;j<optLen; j++){
			var theLetter = outputText.substr(j, 1)
			
			//alert (theLetter);
			if (checkDuplicate(theLetter)==false){
			  if (theLetter != " ")
				outputCharacterArray.push(theLetter);
				}
		}
		/* */
	}
	function checkDuplicate(ltr){
		var opcLen = outputCharacterArray.length;
		var theResult = false;
		for (i=0; i<opcLen; i++){
			if (outputCharacterArray[i] == ltr)
			  theResult = true;
		}
		return theResult;	  
	}
	//alert(outputCharacterArray);
	$.each(outputCharacterArray, function(i,val){
		keyObj[val] = val;				
	});
	...
	var spaceDelKeys = '<button onclick="spaceClicked()">space</button><button onclick="delClicked()">Del</button>'
...
function loadSet(value){
	...
	var doneStr = '<img  class="donesymbol" id="done' + theNo + '" src="../common/img/feedback_correct_disc.png" border="0">';
	//alert(doneStr);
...
//	var doneStr = '<img  class="doneImg" id="done' + No + '" src="../common/img/letterFbCorrect.png" border="0">';
