...
function loadSet(value){
	...
	for(var i  = 1; i<6; i++){
		...
		$("#dropTarget_" + i).addClass("dropTarget");
		$("#dropTarget_" + i).removeClass("itemCompleted");
		$("#dropTarget_" + i).removeClass("targetDropped");
		
		
		
		$('#dropTargetText_' + i).text(
				$($(xml).find("lang_tl")[(currentSet*numItemsPerSet) + i - 1]).text()
			);
		$('#dropTargetAns_' + i).text(
				$($(xml).find("lang_en")[(currentSet*numItemsPerSet) + i - 1]).text()
			);	
			
		$('#dropTargetAns_' + i).addClass("displayNone");
	}
...
function dropFunction(event, ui ) {
	...
	if(dropTargetNumLookingFor == dropTargetNumGot){
		...
		$(this).find(".dropTargetAns").removeClass("displayNone");
		$(this).addClass("targetDropped");
