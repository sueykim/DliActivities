...
function parseXml(t_xml){
	...
	//check if "TL" or "transliteration" is for drop target text
	dropTargetTL = $(xml).find("content").attr("tl");
	// To display ruby tag
...
function loadSet(value){
	...
	for(var i  = 1; i<6; i++){
		...
		{
			$('#tlText_' + i).text(
				$($(xml).find("lang_trans")[(currentSet*numItemsPerSet) + i - 1]).text()
			);
		}
...
*/	
	if(dropTargetNumLookingFor == dropTargetNumGot){
		...
		timeoutID = window.setTimeout(timerEnd, 1000);
