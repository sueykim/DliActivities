...
var holdTimeout;
var gTotalFamilyMemberWithoutDistractor;//number image to allow user to selected
...
function parseXml(t_xml) {
	...
	isJapanese = $(gXmlEnable21).find("content").attr("target_language") == "Japanese";
	...
	var familyTree = document.createElement("img");
        ...
        $(mSetElem).find("family-member").each(function(){
			...
			if (($(this).find("member_type").text().toLowerCase().replace(/^\s+|\s+$/g,'')) != "distractor"){
				...
				gTotalFamilyMemberWithoutDistractor--;
				...
				var tl_name  = $(this).find("tl_name").text();
				  	if (isJapanese) 
						tl_name = displayRubyTag(tl_name);
...
function checkAnserfromUserSeelected(target) {
	...
	var targetIndex = parseFloat(target.replace ("image_", ""));
	 ...
	 var userAnswer;
	  $(mSetElem).find("family-member").each(function(){
		 var targetName = $(this).find("tl_name").text(); 
		  if (isJapanese) 
						targetName = displayRubyTag(targetName);
			if (parseFloat($(this).attr("position")) == targetIndex)
				userAnswer  = targetName +" ("+ $(this).find("en_name").text() +")";
	  ...
	  //for Homework to get context for homework.
	   var targetName = $(familyMember).find("tl_name").text();
		  if (isJapanese) 
						targetName = displayRubyTag(targetName);
	  var context = targetName +" ("+ $(familyMember).find("en_name").text() +")";
	...
	if (currentTarget == targetIndex) {
		  ...
		  if ($(mSetElem).attr("numberCorrectSelected")) {
					var num= parseFloat($(mSetElem).attr("numberCorrectSelected")) +1;
		    		$(mSetElem).attr("numberCorrectSelected", num);
		} else
		    		$(mSetElem).attr("numberCorrectSelected", "1");
			   ...
			   if ($(familyMember).find("tl_phrase").text() !="") {
						  		...
						  		document.getElementById("audioTextPhase").setAttribute("dir", $(familyMember).find("tl_name").attr("dir"));
								   var targetName = $(familyMember).find("tl_name").text();
		 								 if (isJapanese) 
												targetName = displayRubyTag(targetName);
				  		 		 document.getElementById("audioTextPhase").innerHTML= targetName;
				}
		...
		if (parseFloat($(mSetElem).attr("numberCorrectSelected")) >0) {
		      var  finalScoreForSet = (parseFloat($(mSetElem).attr("numberCorrectSelected"))/gTotalFamilyMemberWithoutDistractor); 
	         $(mSetElem).attr("scoreForSet",finalScoreForSet);
		}
		var totalScoreforAllSets=0;
		$(gXmlEnable21).find("family-tree").each(function(){
		...
		finalScore =  Math.ceil((totalScoreforAllSets/gTotalSetNumber)*100);
	 ...
	 if (type == "true") {
						  ...
						  if ($(familyMember).find("tl_phrase").text() !="") {
								  ...
								  var targetPhrase = $(familyMember).find("tl_phrase").text();
		 								 if (isJapanese) 
												targetPhrase = displayRubyTag(targetPhrase);
				  		 		 document.getElementById("audioTextPhase").innerHTML= targetPhrase;
						  }else {
						  		document.getElementById("audioTextPhase").setAttribute("dir", $(familyMember).find("tl_name").attr("dir"));
								  var targetName = $(familyMember).find("tl_name").text();
		 								 if (isJapanese) 
						  ...
						  $("#feedbackBody").html($(familyMember).find("en_name").text());
		 	  ...
		 	  var messageFeedBack =document.createElement("div");
	    		  messageFeedBack.id ="messageFeedBack";
				  	if ($(familyMember).attr("answerAttempts") && (parseFloat($(familyMember).attr("answerAttempts")) < 3)) { 
				 		 ...
				 		 messageFeedBack.innerHTML="<div>Hint: "+ $(familyMember).find("hint").text() +"</div>";
				    		 holdTimeout= setTimeout(function(){$("#feedBackDiv").css({display:"none"}); },3000);
					} else {
						 messageFeedBack.setAttribute("dir", $(familyMember).find("en_name").attr("dir"));
				 		 messageFeedBack.innerHTML= $(familyMember).find("en_name").text();
					}
