   ...
   feedBackDiv.appendChild(feedbackBottom);
	...
	if (!statusParameters) {
		...
		xmlPath 	= "activityData/";
			...
			xmlFilename = xmlPath + "xml/ja_l10_02.xml"; //xml url
	...
	var familyTree = document.createElement("img");
        ...
        $(mSetElem).find("family-member").each(function(){
			...
			if (($(this).find("member_type").text().toLowerCase().replace(/^\s+|\s+$/g,'')) != "distractor"){
				...
				var tl_name  = "";
				if (!isJapanese) {
					tl_name  = $(this).find("tl_name").text();
				}
				else {
					// To display ruby tag
					tl_name  = displayRubyTag($(this).find("tl_name").text());
				}
			   ...
			   if ($(familyMember).find("tl_phrase").text() !="") {
								...
								// document.getElementById("audioTextPhase").innerHTML= $(familyMember).find("tl_phrase").text();
								 if (!isJapanese) {
									...
									// To display ruby tag
									document.getElementById("audioTextPhase").innerHTML= displayRubyTag($(familyMember).find("tl_phrase").text());
								}
				}else {
								...
								if (!isJapanese) {
									...
									// To display ruby tag
									document.getElementById("audioTextPhase").innerHTML= displayRubyTag($(familyMember).find("tl_name").text());
								}
				}
		  ...
		  $("#titleFeedBack").html("<img src='images/feedback_incorrect.png' class='feedbackImage'/>");
		 	  var messageFeedBack =document.createElement("div");
				...
				if (!isJapanese) {
					...
					// To display ruby tag
					messageFeedBack.innerHTML= displayRubyTag($(familyMember).find("hint").text());
				}
