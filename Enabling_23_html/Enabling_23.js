...
function loadClues(){
	...
	$.each(["up","down","left","right"], function(i, v){
		if($("clue[direction=" + v +"]").length > 0){
			...
			var labelTxt = v == 'right' || v == 'left' ? 'across' : v ;
	...
	$($(keywordLetterXml).find("keywords")[currentSet]).find("keyword").each(function(index){
		...
		if($(this).find("letter[row=" + (rowIndex + 1) + "][col=" + (colIndex + 1) + "]").length > 0){
			...
			if(keywordFilled){
				...
				jKeyword.attr("correctValueText", correctValueText);
				var selectValueText = "";
				...
				if($(this).attr("timesTried") == undefined){
					...
					$(this).attr("timesTried",
						parseInt($(this).attr("timesTried")) + 1
					);	
				}
				if (homeworkStatus){
				...
				//	alert('jKeyword: ' + jKeyword.attr("correctValueText") + 'jKeyword.attr("completed"): ' + jKeyword.attr("completed"))
				}else{
