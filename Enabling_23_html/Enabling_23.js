...
function keyboardLayout() {
	...
	$(
			$(xml).find("tl_text").text().replace(/[ ]/g, "").toLowerCase()
					.split("").filter(function(itm, i, a) {
	...
	$.each(keyObj, function(i, val) {
		kbHtml += '<div class="keyboardLetter noSelect">' + val + '</div>';
	});
...
function loadSet(value) {
	...
	jSection.find("en_word").each(
			function(i) {
				...
				$("#audioList").append(
						jSnip.html().replace(/(['"])playBtn_s1/g,
								"$1playBtn_s" + (i + 1)));
			});
			...
			.each(
					...
					function(i, v) {
						if ($("clue[direction=" + v + "]").length > 0) {
							...
							var labelTxt = v == 'right' || v == 'left' ? 'across'
									: v;
							$("#clues").append(
									"<p class='clueDirectionLabel'>"
											+ capitaliseFirstLetter(labelTxt)
											+ ":</p>");
...
function playEn(value) {
	...
	var mediaFile = $(
			$(jSection.find("keyword")[keywordIndex]).find(
					"> tl_media media_file")).text();

	switch ($(
			$(jSection.find("keyword")[keywordIndex]).find(
					"> tl_media media_type")).text()) {
	case "audio":
		audio_play_file(removeFileExt(mediaFile), mediaPath);
		break;
	case "video":
		loadVideoNoPlayYet(mediaPath, removeFileExt(mediaFile));
		document.getElementById('videoTag').addEventListener('ended',
...
function playClue(value) {
	...
	var mediaFile = $(
			$(jSection.find("keyword")[keywordIndex]).find(
					"clue tl_media media_file")).text();

	switch ($(
			$(jSection.find("keyword")[keywordIndex]).find(
					"clue tl_media media_type")).text()) {
	case "audio":
		audio_play_file(removeFileExt(mediaFile), mediaPath);
		break;
	case "video":
...
function dropFunction(event, ui) {
	...
	// Load letter
	$(
			$($($("#grid").find("tr")[rowIndex]).find("td")[colIndex]).find(
					".letter")).text(dragLetter).removeClass("invisible")
			.removeClass("redLetter");

	// Loop through all keywords
	$($(keywordLetterXml).find("keywords")[currentSet])
			.find("keyword")
			.each(
					function(index) {
						// Does this keyword contain the current dropped cell
						if ($(this).find(
								"letter[row=" + (rowIndex + 1) + "][col="
										+ (colIndex + 1) + "]").length > 0) {
							// The keyword contains the cell
							var jKeyword = $(jSection.find("keyword")[index]);
							jKeyword.attr("index", index);
							...
							$(this)
									...
									.each(
											function() {
												var letterRowIndex = $(this)
														.attr("row");
												var letterColIndex = $(this)
														.attr("col");
							...
							if (keywordFilled) {
								var correctValueText = "";

								$(this).find("letter").each(function() {
									correctValueText += $(this).attr("letter");
								});
								jKeyword.attr("correctValueText",
										correctValueText);
								var selectValueText = "";
								// Check that the letters match. If they don't
								// flag them
								$(this)
										.find("letter")
										.each(
								...
								if (homeworkStatus) {
									// alert('hw')
									logStudentAnswer(currentSet + ":"
											+ ($(this).index() + 1),
											correctValueText, selectValueText);

								}
								if ($(this).attr("timesTried") == undefined) {
									$(this).attr("timesTried", 1);
								} else {
									$(this)
											.attr(
													"timesTried",
								...
								if (keywordPassed) {
									...
									showFeedback("correct", jKeyword.find(
											"feedback").text());
									// For homework
									if (homeworkStatus) {
										sendHWdata(1, jKeyword);
									}
									// alert('jKeyword: ' +
									// jKeyword.attr("correctValueText") +
									// 'jKeyword.attr("completed"): ' +
									// jKeyword.attr("completed"))
								} else {
									if (jKeyword.attr("hintCount") == undefined) {
										jKeyword.attr("hintCount", "1");
										showFeedback("incorrect", jKeyword
												.find("hint_1").text());
...
function sendHWdata(N, KW) {
	...
	answerAttemptsNum = jKword.attr("hintCount");
	if (answerAttemptsNum == undefined)
		answerAttemptsNum = "0"
	questionID = parseInt(currentSet.toString()) + "0" + jKword.attr("index");
	answer = N == 0 ? 'incorrect' : 'correct';
	context = jKword.attr("correctValueText");
...
function autoFillKeyword(keyword) {
	$(keyword)
			...
			.each(
					function(j) {
						var jTr = $($("#grid").find("tr")[parseInt($(this)
								.attr("row")) - 1]);
						var jTd = $(jTr.find("td")[parseInt($(this).attr("col")) - 1]);

						jTd.removeClass("invisible");

						jTd.find(".letter").text($(this).attr("letter"))
								.removeClass("redLetter");
					});
}
var keywordXML
...
function loadKeywordXml() {
	...
	$(keywordLetterXml).find("keyword").each(
			function() {
				var randLetterIndex = randomRange(0,
						$(this).find("letter").length - 1);
			...
			.each(
					function(index) {
						...
						$(this)
								...
								.each(
										function() {
											var letterRowIndex = $(this).attr(
													"row");
											var letterColIndex = $(this).attr(
													"col");
			...
			.each(
					function(i) {
						...
						$(this)
								...
								.each(
										function(j) {
											var jTr = $($("#grid").find("tr")[parseInt($(
													this).attr("row")) - 1]);
											var jTd = $(jTr.find("td")[parseInt($(
													this).attr("col")) - 1]);

											jTd.removeClass("invisible");

											if (j == 0) {
												jTd.find(".number").text(
														$(this).attr("number"));
											}

											jTd.find(".letter").removeClass(
													"invisible");

											if (randLetterIndex != undefined
													&& j == parseInt(randLetterIndex)) {
												jTd.find(".letter").text(
														$(this).attr("letter"));
											}
										});
...
function constructGrid() {
	...
	for (var i = 0; i < rows; i++) {
		...
		for (var j = 0; j < cols; j++) {
			cellHtml += "<td class='invisible cell'>" + $("#cellSnip").html()
					+ "</td>";

			/*
			 * cellHtml += "<td>" + i + "," + j; cellHtml += "<td class='invisible cell'>" + "<div
			 * class='containerDiv'>" + "<span class='number'></span>" + "<span
			 * class='letter invisible'></span>" + "<span
			 * class='droppedLetter'></span>" + "</div>" + "</td>";
			 */
		}
	...
	/*
	 * cellWidth = $("#grid").find("td").width(); cellHeight =
	 * $("#grid").find("td").height();
	 * 
...
function closeFeedback() {
	...
	if (feedbackCorrectShown) {
		...
		if ($(xml).find("set").length == $(xml).find("set[completed='true']").length
