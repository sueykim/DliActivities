				...
				function() {
					...
					$(document)
							.mousemove(
									function(e) {
										mouseX = e.pageX
												- $("#imageContainer").offset()['left'];
										mouseY = e.pageY
												- $("#imageContainer").offset()['top'];
										// $(output).html(mouseX + "/" +
										// mouseY);
									});
...
function dropFunction(event, ui) {
	var dragId = extractLastLetter($(ui.draggable.find(".dragBubbleText")[0])
			.attr("id"));

	// Play audio
	var iconName = $(jSection.find("icon")[dragId]).text();
	var iconAudio = $($(xml).find("color[icon='" + iconName + "']")).attr(
			"audio");
	...
	for (var i = 0; i < jSection.find("image").length; i++) {
		...
		if (canvas == undefined) {
			// not quite sure why this happens. Maybe the canvas images haven't
			// loaded yet?
		...
		if (p[3] > 0) {
			...
			if (i == dragId) {
				...
				// Check for complete
				if ($(".ui-draggable-disabled").length == jSection
						.find("image").length
	...
	if (homeworkStatus) {
		logStudentAnswer(currentSet, $(jSection.find("icon")[lastMatchId])
				.text(), iconName);
	}
	...
	if (jIcon.attr("timesTried") == undefined) {
		...
		jIcon.attr("timesTried", parseInt(jIcon.attr("timesTried")) + 1);
	}
	if (homeworkStatus) {
		logStudentAnswerAttempts(currentSet, jIcon.attr("timesTried"));
	}
}
...
function showFeedback(value, textInput) {
	...
	/*
	 * $('#feedback').animate( { left: '380px', top: '200px', //width: '400px',
...
function closeFeedback() {
	...
	if (setCompletedShown) {
		if (!activityCompletedShown
				&& $(xml).find("set[completed='true']").length == $(xml).find(
						"set").length) {
	...
	/*
	 * $('#feedback').css( { left: '580px', top: '250px', width: 0, height: 0 } );
	 */
}
...
function loadSet(value) {
	...
	// Load whole image
	$("#wholeImage").attr("src",
			mediaPath + "png/" + jSection.find("background").attr("source"));
	$("#outline").attr("src",
			mediaPath + "png/" + jSection.find("outline").attr("source"));

	// Get snippet
	var snippetDragBubble = $("#dragBubble_id").outerHTML();
	snippetDragBubble = snippetDragBubble.replace("[dragBubble]", "dragBubble");
	snippetDragBubble = snippetDragBubble.replace("[dragBubbleText]",
			"dragBubbleText");
	...
	if (jSection.attr("completed") != "true") {
		...
		for (var i = 0; i < jSection.find("icon").length; i++) {
			dragBubblesHTML += snippetDragBubble.replace("dragBubble_id",
					"dragBubble_" + i).replace("dragBubbleText_textId",
					"dragBubbleText_" + i);
		}
		...
		$('.dragBubble').draggable(
				{
					...
					start : function() {
						...
						$(this).data('draggable').offset.click.left = $(this)
								.width() / 2 - 20;
					}
				});
		...
		for (var i = 0; i < jSection.find("icon").length; i++) {
			...
			var iconText = $($(xml).find("color[icon='" + iconName + "']"))
					.text();
			var iconBackground = $(
					$(xml).find("color[icon='" + iconName + "']")).attr(
					"source");
...
function loadImages() {
	...
	img.src = mediaPath + "png/"
			+ $(jSection.find("image")[canvasLoadNum]).attr("source");
}
