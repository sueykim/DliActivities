...
function loadSet() {
	...
	$('#dropContentArea, #videoDiv, #dragPanel, #feedbackContent').html("");
	 $("#setText").html((gCurrentSet+1) + '/' + gTotalSetNumber);
	  ...
	  var i=0;
		...
		if (mSectionElem.attr("completed") == "true") {
				...
				$(".dropBubble").css({"visibility":"visible", "color": "#7b0026" });
		   ...
		   if (gTotalsectionCompleted == gTotalSetNumber) {
													...
													$("#feedbackContent").html("<div class='corrected'>The Activity is completed.</div>");
			} else {
				$("#feedbackContent").css({"display": "block"});
			    $("#feedbackContent").html("<div><span class='corrected'>The Set " +(gCurrentSet+1) + '/' + gTotalSetNumber +" is completed.</span></div>");
		   }
		}
...
function dropFunction(event, ui ) {
	...
	var userAnswer = "";
				...
				if(name.indexOf("correct") > -1 ){
						...
						$("#feedbackContent").css({"display": "block"});
						$("#feedbackContent").html("<div><div>Feedback: <span class='corrected'>The answer is correct.</span></div><div id='engFeedback'>" + $($(mSectionElem).find("feedback")[i]).text() + "</div></div>");
						...
						setTimeVariable = setTimeout(function(){ $("#feedbackContent").fadeOut(); },5000);
						
						
				} else {
					 userAnswer = $(ui.draggable).html() +" -- Incorrect!"
					$("#feedbackContentt").stop(true, true);
						$("#feedbackContent").css({"display": "block"});
						$("#feedbackContent").html("<div>Feedback: <span class='inCorrect'>That answer is incorrect.</span></div>");
						setTimeVariable = setTimeout(function(){ $("#feedbackContent").fadeOut(); },2000);
				}
