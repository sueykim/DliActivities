 ...
 var feedback= document.createElement("div");
   ...
   feedback.appendChild(feedbackBottom);
	...
	if (!statusParameters) {
			...
			xmlFilename =   xmlPath  + "xml/ja_l05_05.xml"; //xml url
...
function showFeedback(type, string) {
 ...
 var mSectionElem = $(gXmlEnable22).find("section").eq(gCurrentSet);
    ...
    if (type == "hint") 
       $("#titleFeedBack").html("<div class='feebackTitle'>HINT</div>");
		
    if (type == "feedback") 
       $("#titleFeedBack").html("<div class='feebackTitle'>FEEDBACK</div>");
	
    if (type == "alert") 
        $("#titleFeedBack").html("<div class='feebackTitle'>WARNING MESSAGE!</div>");
	
    if (type == "activity_completed") {
		 $("#titleFeedBack").html("<div class='feebackTitle'>ACTIVITY COMPLETED!</div>");
    }
	
    if (type == "set_completed") {
	        $("#titleFeedBack").html("<div class='feebackTitle'>SET COMPLETED!</div>");
