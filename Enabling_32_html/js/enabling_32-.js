...
$(document).ready(function() {
  ...
  document.getElementById("activityContainer").appendChild(audioPlayer);
   ...
   document.getElementById("activityContainer").appendChild(flashAudio);
	...
	string += '<button class="btn btn_item" id="itemDiv" title="Item" style="display:none;">ITEM:&nbsp;</button>';
    string += '<button class="btn btn_set_value off" id="itemText" title="Item" style="display:none;">1/5</button></div>';
	...
	$(function() {
		...
		if ($(itemXML).attr("itemCompleted") == "true"){
			...
			if ($(itemXML).attr("itemPartialCompleted") != "true")  {
				...
				context += " -- " + $(itemXML).find("tl_word").text();
