...
$(document).ready(function () {
	...
	cssFilename = "styles/RTLMobilePhone_default.css";
...
function parseXml(t_xml)
{
	...
	//Load Selectable List
	var tally = 0;
	$(xml).find("item").each(function(){
		var pn = translatePhoneNum($(this).attr("english"));
		$("#selectable").append("<li class='phoneNum' id='" + tally + "'>" + pn + "</li>");
		tally++;
	});
	...
	$( "#selectable" ).selectable({
			...
			selected: function (event, ui) {
		  		//Find index
		  		loadSet(Number($(this).find('.ui-selected').attr("id")));
			}
		}
	);
	
	//Have the first set selected
	setSelected(0);
...
function callPressed(){
	...
	if(str == currentPhoneNumber){
		...
		document.getElementById('wholeClickGuard').style.display = "block";
...
function correctTimerEnd(){
	...
	document.getElementById('wholeClickGuard').style.display = "none";
	document.getElementById('clickGuard').style.display = "block";
	btnLock = false;
}
