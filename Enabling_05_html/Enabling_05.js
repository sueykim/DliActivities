$(document).ready(function() {
	...
	$(".dropTarget").click(function(){
		var clickTargetNumGot = extractLastLetter($(this).attr("id"));
		
		//Play audio
		var file_audio = $(jSection.find("file_audio")[clickTargetNumGot - 1]).text();
...
function loadSet(value){
	...
	for(var i  = 1; i<6; i++){
		...
		if (!isJapanese) {
			$('#dropTargetText_' + i).html($(jSection.find("phrase_tl")[i-1]).text());
		}
		else {
			// To display ruby tag
			$('#dropTargetText_' + i).html(displayRubyTag($(jSection.find("phrase_tl")[i-1]).text()));
		}		
	}
