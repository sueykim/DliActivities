...
function parseXml(t_xml){
	...
	for(var i=0; i< numSets; i++){
                ...
                if (!isJapanese) {
					...
					// To display ruby tag
					note = displayRubyTag($(xml).find("set").eq(i).find("psg").find("note").text());
				}
	        ...
	        for (var j=0; j< numPhrase; j++)
	          {
				...
				if (!isJapanese) {
					...
					// To display ruby tag
					txt =  displayRubyTag($(xml).find("set").eq(i).find("phrase").eq(j).find("txt").text());
					cfb =  displayRubyTag($(xml).find("set").eq(i).find("phrase").eq(j).find("crrt_fdbk").text());
					wfb =  displayRubyTag($(xml).find("set").eq(i).find("phrase").eq(j).find("wrng_fdbk").text());
				}				  
...
//  	updateNavButtons()
	...
	if(actGuard[currentSet] == 1)
		...
		$(".playBtnImg").addClass('white');
		$(".playBtnImg").each(function(index){
          $(this).click(function(){
             $(".playBtnImg").removeClass('blue');
             $(".playBtnImg").addClass('white');
             $(this).removeClass('white');
             $(this).addClass('blue');
             $("#txtGuard").hide();
             ...
             var selId = $(this).attr('id')
			 selIdNum = parseInt(selId.substr(selId.length-1))
               })
