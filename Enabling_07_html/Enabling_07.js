...
function dropFunction(event, ui ){	
	...
	if(dragBtnIndex == indexNum){
		...
		$("#caption_2_text").css("display","block"); //not show 2nd text
	}else{
...
function exchangeSolved(){
	...
	$("#caption_2_text").css("display","block");
...
function loadExchange(value){
	...
	if (!isJapanese) {
		$("#caption_1_text").html($(jItem.find("tl_prompt_text")).text());
		$("#caption_2_text").html($(jItem.find("tl_response_text")).text());
	}
	else {
		// To display ruby tag
...
function checkAnswers(){
	...
	//$("#feedbackText").html("Homework//answerAttempts: " + answerAttemptsNum.toString() + " - " + questionID);
