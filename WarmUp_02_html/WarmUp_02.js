	...
	if (dropBtnLetter == dragBtnLetter) {
		...
		if (nCountTrials == missingLetterIdsTrialsArray.length) {
			...
			if (!isJapanese) {
				...
				// To display ruby tag
				showFeedback("correct", displayRubyTag($($(xml).find("feedback_l1")[currentSet]).text()));
			}
		}
	} else {
		...
		if ((nCountTrials == (missingLetterIdsTrialsArray.length - 1)) && (nCountSecondIncorrectTrials > 0)) {
			...
			if (!isJapanese) {
				...
				// To display ruby tag
				showFeedback("incorrect", displayRubyTag($($(xml).find("feedback_l1")[currentSet]).text()));
			}
		...
		if (missingLetterIdsTrialsArray[dropBtnIndex] == 1) {
			...
			if (!isJapanese) {
				...
				// To display ruby tag
				showFeedback("incorrect", displayRubyTag($($(xml).find("hint_l1")[currentSet]).text()));
			}
